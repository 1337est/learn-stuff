...
#		Here is represents the object files needed to build the target.
#	$(CC): Compiler command (g++)
#	$(CFLAGS): Compiler flags, including debugging info, C++ standard, warnings, etc
#	-o $@: -o specifies the output file name. $@ is an automatic variable that
#		represents the target name ('main' in this case).
#	$^: Special automatic variable that represents all the prerequisites of the rule.
